...
public class TownCommandExecutor implements CommandExecutor
{
	...
	public TownCommandExecutor()
	{
		...
		commands.add(new NewTownCommand());
		commands.add(new DeleteTownCommand());
		commands.add(new TownInfoCommand());
		commands.add(new TownJoinCommand());
		commands.add(new ListCommand());
		commands.add(new TownQuitCommand());
		commands.add(new TownPromoteCommand());
		commands.add(new TownStatusCommand());
		commands.add(new MoneyCommand());
		commands.add(new TownSetCommand());
		commands.add(new TownFlagCommand());
		commands.add(new TownRuleCommand());
		commands.add(new TownMailCommand());
		commands.add(new TownVaultCommand());
		commands.add(new TownDemoteCommand());
		// TODO: commands.add(new ConfirmCommand());
		
		// log to StopWatch
        sw.setLoad("townCommandExecutor", System.nanoTime() - start);
	}
		...
		while (index < guardCmdArray.length && tempArgs.length > 0)
		{
			...
			if(tempArgs[0].equalsIgnoreCase(townCmd.name))
			{
				...
				else
				{
					...
					// log to StopWatch
			        sw.setLoad("townOnCommand", System.nanoTime() - start);
